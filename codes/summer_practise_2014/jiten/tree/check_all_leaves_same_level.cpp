...
bool check_level(node* root,int *h)
{
	int lh,rh;
	if(root==NULL)
	{
	...
	bool c_l=check_level(root->left,&lh);
	bool c_r=check_level(root->right,&rh);
...
int main()
{
	...
	int height;
	if(check_level(root,&height))
		printf("All are at same level\n");
	else
		printf("nups dude\n");
	return 0;
}
